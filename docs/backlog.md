...
| C-013 | Pet expression summarizes the most urgent state across all live sessions (error > waiting > working) | Saves scanning rows when many sessions are live; cut from 001 as redundant with per-row state, uncut at spec round 004 once the pet itself turned out to be unbuilt — absorbed into C-005 | scope 001 (reversed, spec round story 004) | Planned | 001 |
